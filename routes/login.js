...
router.get('/redirect', async (ctx, next) => {
  try {
    ...
    url = `https://graph.qq.com/user/get_user_info?access_token=${token}&oauth_consumer_key=${
      loginConfig.appid
    }&openid=${openid}`
    res = await request(url)
    res = JSON.parse(res)
...
router.get('/', async (ctx, next) => {
  if (configs.website_info.debug) {
    ...
    var url = `https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=101383430&redirect_uri=${
      loginConfig.redirect_url
