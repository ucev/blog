  ...
  async getall ({ orderby = { lb: 'id', asc: 'asc' }, queryfields = ['*'] }) {
    try {
      ...
      var labels = await conn.query(
        `select ?? from ${this.dbname} order by ${conn.escapeId(
          orderby.lb
        )} ${this.__queryorder(orderby.asc)}`,
        [[queryfields]]
      )
      conn.end()
